...
/*采集样本 Description: compute the coordinate of positive and negative sample image templates
Arguments:
...
//跟踪处理，传入上一帧跟踪到的box，来处理新的一帧  
void CompressiveTracker::processFrame(Mat& _frame, Rect& _objectBox)
{
	// predict
	//在上一帧跟踪到的boxbox周围，采集需要检测的box框  
	sampleRect(_frame, _objectBox, rSearchWindow, detectBox);//对应CT
	//sampleRect(_frame, _objectBox, rSearchWindow,detectBox,4);//对应FCT，若此行被注释，对应第二采样也被注释
	//计算这一帧的积分图  
	integral(_frame, imageIntegral, CV_32F);
